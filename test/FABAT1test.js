const FabaTokenIco = artifacts.require('./FABATest1.sol');
...
contract('FabaTokenIco', accounts => {
  ...
  const createToken = () => FabaTokenIco.new(fundsWallet, timeController.currentTimestamp(), minCap, maxCap);

  // REQ001: Basic ERC20 “Faba Test Token” with symbol of “FABAT1”, 
  ...
  it('should have initial supply of 1,000,000 units assigned to funds wallet', async () => {
    const fabaToken = await createToken();
    const expectedSupply = toWei(16000000);

    const totalSupply = await fabaToken.totalSupply();
    assert.equal(totalSupply, expectedSupply, 'Total supply mismatch');

    const fundsWalletBalance = await fabaToken.balanceOf(fundsWallet);
  ...
  it('should open at startTimestamp', async () => {
    ...
    const fabaToken = await FabaTokenIco.new(fundsWallet, startTimestamp, minCap, maxCap);

    // should be closed
    await assertExpectedError(fabaToken.sendTransaction(transaction(buyerOneWallet, oneEth)));

    await timeController.addSeconds(3600);

    // should be open
    await fabaToken.sendTransaction(transaction(buyerOneWallet, oneEth));
  });

  it('should last 4 weeks if the goal is reached and allow token transfers afterwards', async () => {
    const fabaToken = await createToken();

    await fabaToken.sendTransaction(transaction(buyerOneWallet, minCap));
    await timeController.addDays(4 * 7);

    // should be closed
    await assertExpectedError(fabaToken.sendTransaction(transaction(buyerTwoWallet, oneEth)));

    const totalRaised = await fabaToken.totalRaised();
    assert.equal(totalRaised.toNumber(), minCap, 'Total raised amount mismatch')

    // REQ004 should allow token transfer
    await fabaToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet });
  });
  ...
  it('should last more then 4 weeks if the goal is not reached and allow token transfers after closing', async () => {
    ...
    await fabaToken.sendTransaction(transaction(buyerTwoWallet, minCap - oneEth));

    // should be closed
    await assertExpectedError(fabaToken.sendTransaction(transaction(buyerThreeWallet, oneEth)));
  ...
  it('should close after the max cap is reached and allow token transfers afterwards', async () => {
    ...
    await fabaToken.sendTransaction(transaction(buyerTwoWallet, maxCap));
  ...
  it('should transfer raised funds to fundsWallet after each payment', async () => {
    ...
    assert.equal(ethBalance(fabaToken.address), 0, 'Contract balance mismatch');
    assert.equal(ethBalance(fundsWallet), expectedBalanceGrowth(oneEth), 'Funds wallet balance mismatch');

    await fabaToken.sendTransaction(transaction(buyerTwoWallet, oneEth));
  ...
  it('should not allow token transfers before ICO', async () => {
    ...
    await assertExpectedError(fabaToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet }));
  });
  ...
  it('should be sold with +50% bonus during the first week', async () => {
    ...
    const buyerOneBalance = await fabaToken.balanceOf(buyerOneWallet);
